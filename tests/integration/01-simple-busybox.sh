...
TEST_NAME="simple-busybox"
BUNDLE_NAME="busybox-bundle"
...
case "${TEST_PHASE:-build}" in
    build)
        ...
        # Create the bundle
        export BUNDELF_BINARIES="busybox"
        
        echo "Creating BusyBox bundle in $BUNDELF_CODE_PATH..."
    ...
    verify)
        ...
        verify_bundle_structure "busybox" "$BUNDELF_MERGE_BINDIRS"

        # 2. Test basic BusyBox functionality
        verify_bundle_execution "busybox" "ls --help" "List directory contents" "$BUNDELF_MERGE_BINDIRS"
        verify_bundle_execution "busybox" "echo test" "test" "$BUNDELF_MERGE_BINDIRS"

        # 3. Test BusyBox applet functionality
        verify_bundle_execution "busybox" "true" "" "$BUNDELF_MERGE_BINDIRS"

        rel_path=$(find_binary_path "busybox" "$BUNDELF_MERGE_BINDIRS") || exit 1
